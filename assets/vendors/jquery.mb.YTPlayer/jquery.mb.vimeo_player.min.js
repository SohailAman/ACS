...
! function($) {
    jQuery.vimeo_player = {
        ...
        buildPlayer: function(options) {
            ...
            return script.src = "//player.vimeo.com/api/player.js", script.onload = function() {
                ...
                if (jQuery.extend(vimeo_player.opt, jQuery.vimeo_player.defaults, options, property), eval(vimeo_player.opt.loop) && (vimeo_player.opt.loop = 9999), vimeo_player.isRetina = window.retina || window.devicePixelRatio > 1, vimeo_player.canGoFullScreen = !(jQuery.browser.msie || jQuery.browser.opera || isIframe()), vimeo_player.canGoFullScreen || (vimeo_player.opt.realfullscreen = !1), vimeo_player.isAlone = !1, vimeo_player.hasFocus = !0, vimeo_player.videoID = this.opt.videoURL ? get_vimeo_videoID(this.opt.videoURL) : !!$vimeo_player.attr("href") && get_vimeo_videoID($vimeo_player.attr("href")), vimeo_player.isSelf = "self" == vimeo_player.opt.containment, vimeo_player.opt.containment = "self" == vimeo_player.opt.containment ? jQuery(this) : jQuery(vimeo_player.opt.containment), vimeo_player.isBackground = vimeo_player.opt.containment.is("body"), !vimeo_player.isBackground || !vimeo_player.backgroundIsInited) {
                    vimeo_player.canPlayOnMobile = vimeo_player.isSelf && 0 === jQuery(this).children().length, vimeo_player.isSelf || $vimeo_player.hide();
                    ...
                    if (wrapper.css({
                            ...
                            overflow: "hidden",
                            opacity: 0
                        }), vimeo_player.playerBox = jQuery("<iframe/>").attr("id", playerID).addClass("playerBox"), vimeo_player.playerBox.css({
                            position: "absolute",
                            zIndex: 0,
                            frameBorder: 0,
                            overflow: "hidden",
                        ...
                        }), vimeo_player.opt.containment.prepend(wrapper), jQuery.browser.mobile && !vimeo_player.canPlayOnMobile) return vimeo_player.opt.mobileFallbackImage && wrapper.css({
                    ...
                    wrapper.append(vimeo_player.playerBox), vimeo_player.opt.containment.children().not("script, style").each(function() {
                        ...
                        position: "relative"
                    }), vimeo_player.videoWrapper = wrapper, jQuery.browser.mobile || (vimeo_player.playerBox.after(overlay), vimeo_player.overlay = overlay), vimeo_player.isBackground || overlay.on("mouseenter", function() {
                        ...
                        vimeo_player.player = new Vimeo.Player(playerID, options), vimeo_player.player.ready().then(function() {
                            function start() {
                                vimeo_player.isReady = !0, vimeo_player.opt.mute && setTimeout(function() {
                                    $vimeo_player.v_mute()
                                }, 1), vimeo_player.opt.showControls && jQuery.vimeo_player.buildControls(vimeo_player), vimeo_player.opt.autoPlay ? setTimeout(function() {
                                    $vimeo_player.v_play(), VEvent = jQuery.Event("VPStart"), $vimeo_player.trigger(VEvent)
                            ...
                            vimeo_player.opt.startAt ? (vimeo_player.player.play().then(function() {
                                ...
                                vimeo_player.state = 1, $vimeo_player.trigger("change_state"), vimeo_player.controlBar && vimeo_player.controlBar.length && vimeo_player.controlBar.find(".vimeo_player_pause").html(jQuery.vimeo_player.controls.pause), "undefined" != typeof _gaq && eval(vimeo_player.opt.gaTrack) && _gaq.push(["_trackEvent", "vimeo_player", "Play", vimeo_player.videoID]), "undefined" != typeof ga && eval(vimeo_player.opt.gaTrack) && ga("send", "event", "vimeo_player", "play", vimeo_player.videoID), VEvent = jQuery.Event("VPPlay"), VEvent.error = data, $vimeo_player.trigger(VEvent)
                                ...
                                if (vimeo_player.opt.addRaster) {
                                    var l = "dot" == vimeo_player.opt.addRaster ? "raster-dot" : "raster";
                                    vimeo_player.overlay.addClass(vimeo_player.isRetina ? l + " retina" : l)
                                ...
                                var m = vimeo_player.opt.stopAt || a.duration - .5;
                                a.seconds >= m && (vimeo_player.loop = vimeo_player.loop || 0, vimeo_player.opt.loop && vimeo_player.loop < vimeo_player.opt.loop ? ($vimeo_player.v_seekTo(vimeo_player.opt.startAt), vimeo_player.loop++) : ($vimeo_player.v_pause(), vimeo_player.state = 0, $vimeo_player.trigger("change_state"))), VEvent = jQuery.Event("VPTime"), VEvent.time = a.seconds, $vimeo_player.trigger(VEvent)
                            })
        ...
        play: function() {
            ...
            a.player.play(), setTimeout(function() {
        ...
        setVolume: function(a) {
            var b = this.get(0);
            return a || b.opt.vol || !b.isMute ? !a && !b.isMute || a && b.opt.vol == a ? b.isMute ? jQuery(b).v_mute() : jQuery(b).v_unmute() : (b.opt.vol = a, b.player.setVolume(b.opt.vol), b.volumeBar && b.volumeBar.length && b.volumeBar.updateSliderVal(100 * a)) : jQuery(b).v_unmute(), this
        },
        ...
        mute: function() {
            var a = this.get(0);
            if (!a.isMute) {
                a.isMute = !0, a.player.setVolume(0), a.volumeBar && a.volumeBar.length && a.volumeBar.width() > 10 && a.volumeBar.updateSliderVal(0);
                var b = jQuery("#controlBar_" + a.id),
        ...
        unmute: function() {
            var a = this.get(0);
            if (a.isMute) {
                a.isMute = !1, jQuery(a).v_set_volume(a.opt.vol), a.volumeBar && a.volumeBar.length && a.volumeBar.updateSliderVal(a.opt.vol > .1 ? a.opt.vol : .1);
                var b = jQuery("#controlBar_" + a.id),
                    c = b.find(".vimeo_player_muteUnmute");
        ...
        optimizeVimeoDisplay: function(align) {
            ...
            if (vimeo_player.opt.optimizeDisplay) {
                ...
                win.width = el.outerWidth(), win.height = el.outerHeight() + abundance, vimeo_player.opt.ratio = eval(vimeo_player.opt.ratio), vid.width = win.width, vid.height = Math.ceil(vid.width / vimeo_player.opt.ratio), vid.marginTop = Math.ceil(-((vid.height - win.height) / 2)), vid.marginLeft = 0, vimeo_player.playerBox.css({
                    top: 0,
                    opacity: 0,
                    width: 100,
